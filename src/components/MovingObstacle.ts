...
export class MovingObstacle extends Obstacle {
    ...
    private updateZigzagMovement(currentPos: Vector2, deltaTime: number): Vector2 {
        ...
        const zigzagY = this.startPosition.y + Math.sin(this.moveTime * 3) * 30 * verticalSpeed * (deltaTime / 1000);
    ...
    protected checkBounds(): void {
        ...
        if (position.y > GAME_CONFIG.CANVAS.HEIGHT + 500 || distanceFromStart > this.moveRange * 3) {
            this.deactivate();
        }
    }
