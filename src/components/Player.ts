...
import { IGameObject, ComponentConstructor, GameEvent, ICollider, CollisionInfo } from '../types/interface';
...
export class Player extends Component {
    ...
    private jump(): void {
        ...
        this.rigidBody.setVelocity(new Vector2(
            this.rigidBody.getVelocity().x, 
            GAME_CONFIG.PLAYER.JUMP_FORCE
        ));
    ...
    private checkBounds(): void {
        ...
        if (position.x < -GAME_CONFIG.PLAYER.WIDTH / 2) {
            this.transform.setPosition(new Vector2(
                GAME_CONFIG.CANVAS.WIDTH + GAME_CONFIG.PLAYER.WIDTH / 2, 
                position.y
            ));
        } else if (position.x > GAME_CONFIG.CANVAS.WIDTH + GAME_CONFIG.PLAYER.WIDTH / 2) {
            this.transform.setPosition(new Vector2(
                -GAME_CONFIG.PLAYER.WIDTH / 2, 
                position.y
            ));
        }
    ...
    private onCollisionEnter(event: GameEvent): void {
        const { other, collisionInfo } = event.data as { other: ICollider, collisionInfo: CollisionInfo };;
        
        if (this.rigidBody.velocity.y > 0 && this.transform.getWorldPosition().y < other.getGameObject().getComponent(Transform as ComponentConstructor<Transform>)!.getWorldPosition().y) {
            ...
            const otherTransform = other.getGameObject().getComponent(Transform as ComponentConstructor<Transform>);
