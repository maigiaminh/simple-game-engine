...
import { IUIElement, IGameObject, ComponentConstructor } from "../types/interface";
import { Vector2 } from "../utils/Vector2";
import { Component } from "./Component";

export class UIElement extends Component implements IUIElement {
    ...
    public addChild(child: IUIElement): void {
    ...
    public removeChild(child: IUIElement): void {
        if (!(child instanceof UIElement)) {
            throw new Error("Child must be an instance of UIElement");
        }
    ...
    public getParent(): IUIElement | null {
        return this.parent as IUIElement | null;
    }

    public getChildren(): IUIElement[] {
        if (this.children.length === 0) {
            return [];
        }
        
        let childrenCopy: IUIElement[] = [];
        this.children.forEach(child => {
            childrenCopy.push(child as unknown as IUIElement);
        });
    ...
    public deserialize(data: SerializedData): void {
        ...
        this.anchor = data.anchor as UIAnchor || UIAnchor.TopLeft;
        if (Vector2.isVector2D(data.pivot)) {
            this.pivot = new Vector2(data.pivot.x, data.pivot.y);
        }
        if (Vector2.isVector2D(data.size)) {
